...
namespace Zooyard.Core.DynamicProxy
{
    internal class ProxyAssembly
    {
        ...
        public ProxyAssembly()
        {
            ...
            var assemblyName = new AssemblyName("Zooyard.Core.DynamicProxy.Generator") { Version = new Version(1, 0, 0) };
            _ab = AssemblyBuilder.DefineDynamicAssembly(assemblyName, access);
            _mb = _ab.DefineDynamicModule("core");
        }
        ...
        public ProxyBuilder CreateProxy(Type proxyBaseType)
        {
            return CreateProxy("Zooyard.Proxy.", proxyBaseType);
        }
