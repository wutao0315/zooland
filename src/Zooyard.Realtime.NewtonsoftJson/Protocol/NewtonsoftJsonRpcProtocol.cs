...
/// </summary>
public class NewtonsoftJsonRpcProtocol : IRpcProtocol
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="NewtonsoftJsonRpcProtocol"/> class.
    /// </summary>
    public NewtonsoftJsonRpcProtocol() : this(Options.Create(new NewtonsoftJsonRpcProtocolOptions()))
    {
    }
    ...
    public NewtonsoftJsonRpcProtocol(IOptions<NewtonsoftJsonRpcProtocolOptions> options)
    {
    ...
    private RpcMessage? ParseMessage(Utf8BufferTextReader textReader, IInvocationBinder binder)
    {
        try
        {
    ...
    private static void WriteHeaders(JsonTextWriter writer, RpcInvocationMessage message)
    {
    ...
    private static void WriteInvocationId(RpcInvocationMessage message, JsonTextWriter writer)
    {
    ...
    private static RpcMessage BindStreamInvocationMessage(string? invocationId, string target, object?[]? arguments, bool hasArguments, string[]? streamIds)
    {
        if (string.IsNullOrEmpty(invocationId))
        {
            throw new InvalidDataException($"Missing required property '{InvocationIdPropertyName}'.");
    ...
    private static RpcMessage BindInvocationMessage(string? invocationId, string target, object?[]? arguments, bool hasArguments, string[]? streamIds)
    {
        if (string.IsNullOrEmpty(target))
        {
            throw new InvalidDataException($"Missing required property '{TargetPropertyName}'.");
