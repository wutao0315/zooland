...
internal sealed class MessageBuffer : IDisposable
{
    ...
    public MessageBuffer(ConnectionContext connection, IRpcProtocol protocol, long bufferLimit, ILogger logger)
        : this(connection, protocol, bufferLimit, logger, TimeProvider.System)
    {
    }

    public MessageBuffer(ConnectionContext connection, IRpcProtocol protocol, long bufferLimit, ILogger logger, TimeProvider timeProvider)
    {
#if NET8_0_OR_GREATER
    ...
    public ValueTask<FlushResult> WriteAsync(RpcMessage hubMessage, CancellationToken cancellationToken)
    {
        return WriteAsyncCore(hubMessage, _protocol.GetMessageBytes(hubMessage), cancellationToken);
    }

    private async ValueTask<FlushResult> WriteAsyncCore(RpcMessage hubMessage, ReadOnlyMemory<byte> messageBytes, CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (hubMessage is RpcInvocationMessage invocationMessage)
            {
                _totalMessageCount++;
    ...
    internal bool ShouldProcessMessage(RpcMessage message)
    {
        ...
        if (message is not RpcInvocationMessage)
        {
            return true;
        }
