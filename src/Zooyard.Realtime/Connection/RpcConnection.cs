...
/// </summary>
/// <remarks>
/// A <see cref="RpcConnection"/> should be created using <see cref="RpcConnectionBuilder"/>.
...
/// </remarks>
public partial class RpcConnection : IAsyncDisposable
{
    /// <summary>
    ...
    private static readonly MethodInfo _sendStreamItemsMethod = typeof(RpcConnection).GetMethods(BindingFlags.NonPublic | BindingFlags.Instance).Single(m => m.Name.Equals(nameof(SendStreamItems)));
    private static readonly MethodInfo _sendIAsyncStreamItemsMethod = typeof(RpcConnection).GetMethods(BindingFlags.NonPublic | BindingFlags.Instance).Single(m => m.Name.Equals(nameof(SendIAsyncEnumerableStreamItems)));
    ...
    private readonly ConcurrentDictionary<string, InvocationHandlerList> _handlers = new (StringComparer.Ordinal);
    ...
    /// <summary>
    /// Occurs when the <see cref="RpcConnection"/> starts reconnecting after losing its underlying connection.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Occurs when the <see cref="RpcConnection"/> successfully reconnects after losing its underlying connection.
    /// </summary>
    /// <remarks>
    /// The <see cref="string"/> parameter will be the <see cref="RpcConnection"/>'s new ConnectionId or null if negotiation was skipped.
    /// </remarks>
    /// <example>
    ...
    /// <summary>
    /// Indicates the state of the <see cref="RpcConnection"/> to the server.
    /// </summary>
    public RpcConnectionState State => _state.OverallState;

    /// <summary>
    /// Initializes a new instance of the <see cref="RpcConnection"/> class.
    /// </summary>
    /// <param name="connectionFactory">The <see cref="IClientConnectionFactory" /> used to create a connection each time <see cref="StartAsync" /> is called.</param>
    /// <param name="protocol">The <see cref="IRpcProtocol" /> used by the connection.</param>
    /// <param name="endPoint">The <see cref="EndPoint"/> to connect to.</param>
    /// <param name="serviceProvider">An <see cref="IServiceProvider"/> containing the services provided to this <see cref="RpcConnection"/> instance.</param>
    /// <param name="loggerFactory">The logger factory.</param>
    /// <param name="reconnectPolicy">
    /// The <see cref="IRetryPolicy"/> that controls the timing and number of reconnect attempts.
    /// The <see cref="RpcConnection"/> will not reconnect if the <paramref name="reconnectPolicy"/> is null.
    /// </param>
    /// <remarks>
    /// The <see cref="IServiceProvider"/> used to initialize the connection will be disposed when the connection is disposed.
    /// </remarks>
    public RpcConnection(IClientConnectionFactory connectionFactory, IRpcProtocol protocol, EndPoint endPoint, IServiceProvider serviceProvider, ILoggerFactory loggerFactory, IRetryPolicy reconnectPolicy)
    ...
    /// The <see cref="IServiceProvider"/> used to initialize the connection will be disposed when the connection is disposed.
    /// </remarks>
    public RpcConnection(IClientConnectionFactory connectionFactory,
                         ...
                         ILoggerFactory loggerFactory)
    {
        ...
        _logger = _loggerFactory.CreateLogger<RpcConnection>();
        _state = new ReconnectingConnectionState(_logger);

        _logScope = new ConnectionLogScope();

        var options = serviceProvider.GetService<IOptions<RpcConnectionOptions>>();
    ...
    private async Task StartAsyncInner(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            if (!_state.TryChangeState(RpcConnectionState.Disconnected, RpcConnectionState.Connecting))
            {
                throw new InvalidOperationException($"The {nameof(RpcConnection)} cannot be started if it is not in the {nameof(RpcConnectionState.Disconnected)} state.");
            }
            ...
            {
                throw new InvalidOperationException($"The {nameof(RpcConnection)} cannot be started while {nameof(StopAsync)} is running.");
            }
            ...
            _state.ChangeState(RpcConnectionState.Connecting, RpcConnectionState.Connected);
        }
        catch
        {
            if (_state.TryChangeState(RpcConnectionState.Connecting, RpcConnectionState.Disconnected))
            {
    ...
    /// <summary>
    /// Disposes the <see cref="RpcConnection"/>.
    /// </summary>
    ...
    /// <remarks>
    /// This is a low level method for registering a handler. Using an <see cref="RpcConnectionExtensions"/> <c>On</c> extension method is recommended.
    /// </remarks>
    ...
    /// </returns>
    /// <remarks>
    /// This is a low level method for invoking a streaming hub method on the server. Using an <see cref="RpcConnectionExtensions"/> <c>StreamAsChannelAsync</c> extension method is recommended.
    /// </remarks>
    public virtual async Task<ChannelReader<object?>> StreamAsChannelCoreAsync(string methodName, Type returnType, object?[] args, IDictionary<string, string>? headers = null, CancellationToken cancellationToken = default)
    {
        using (_logger.BeginScope(_logScope))
        {
            return await StreamAsChannelCoreAsyncCore(methodName, returnType, args, headers, cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    /// </returns>
    /// <remarks>
    /// This is a low level method for invoking a hub method on the server. Using an <see cref="RpcConnectionExtensions"/> <c>InvokeAsync</c> extension method is recommended.
    /// </remarks>
    public virtual async Task<object?> InvokeCoreAsync(string methodName, Type returnType, object?[] args, IDictionary<string, string>? headers = null, CancellationToken cancellationToken = default)
    {
        using (_logger.BeginScope(_logScope))
        {
            return await InvokeCoreAsyncCore(methodName, returnType, args, headers, cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    /// <remarks>
    /// This is a low level method for invoking a hub method on the server. Using an <see cref="RpcConnectionExtensions"/> <c>SendAsync</c> extension method is recommended.
    /// </remarks>
    public virtual async Task SendCoreAsync(string methodName, object?[] args, IDictionary<string, string>? headers = null, CancellationToken cancellationToken = default)
    {
        using (_logger.BeginScope(_logScope))
        {
            await SendCoreAsyncCore(methodName, args, headers, cancellationToken).ConfigureAwait(false);
        }
    }
...
#pragma warning restore CA2252 // This API requires opting into preview features
        ...
        try
        {
            ...
            {
                // Stateful Reconnect starts with RpcProtocol version 2, newer clients connecting to older servers will fail to connect due to
                // the handshake only supporting version 1, so we will try to send version 1 during the handshake to keep old servers working
                // if the client is not attempting to enable stateful reconnect and therefore does not require a newer RpcProtocol.
...
#pragma warning restore CA2252 // This API requires opting into preview features
            ...
            Log.RpcProtocol(_logger, _protocol.Name, usedProtocolVersion);
        ...
        {
            await SendRpcMessage(startingConnectionState, PingMessage.Instance, cancellationToken).ConfigureAwait(false);
        }
    ...
    private async Task StopAsyncCore(bool disposing)
    {
        ...
        if (reconnectTask.Status != TaskStatus.RanToCompletion)
        {
            ...
            // The StopCts should prevent the RpcConnection from restarting until it is reset.
            _state.ReleaseConnectionLock();
        ...
        try
        {
            ...
            if (connectionState != null)
            {
                ...
                var writeTask = SendRpcMessage(connectionState, CloseMessage.Empty);
...
#pragma warning restore CA2252 // This API requires opting into preview features
            ...
            if (disposing)
            {
                // Must set this before calling DisposeAsync because the service provider has a reference to the RpcConnection and will try to dispose it again
                _disposed = true;
    ...
    /// </returns>
    public virtual IAsyncEnumerable<TResult> StreamAsyncCore<TResult>(string methodName, object?[] args, IDictionary<string, string>? headers = null, CancellationToken cancellationToken = default)
    {
        ...
        var stream = CastIAsyncEnumerable<TResult>(methodName, args, headers, cts);
        var cancelableStream = AsyncEnumerableAdapters.MakeCancelableTypedAsyncEnumerable(stream, cts);
        return cancelableStream;
    }

    private async IAsyncEnumerable<T> CastIAsyncEnumerable<T>(string methodName, object?[] args, IDictionary<string, string>? headers, CancellationTokenSource cts)
    {
        var reader = await StreamAsChannelCoreAsync(methodName, typeof(T), args, headers, cts.Token).ConfigureAwait(false);
    ...
    private async Task<ChannelReader<object?>> StreamAsChannelCoreAsyncCore(string methodName, Type returnType, object?[] args, IDictionary<string, string>? headers, CancellationToken cancellationToken)
    {
        async Task OnStreamCanceled(InvocationRequest irq)
        {
            ...
            try
            {
                if (_state.CurrentConnectionStateUnsynchronized != null)
                {
                    ...
                    await SendRpcMessage(_state.CurrentConnectionStateUnsynchronized, new CancelInvocationMessage(irq.InvocationId), cancellationToken: default).ConfigureAwait(false);
                }
                else
                {
        ...
        try
        {
            ...
            await InvokeStreamCore(connectionState, methodName, irq, args, streamIds?.ToArray(), headers, cancellationToken).ConfigureAwait(false);
    ...
    private async Task CommonStreaming(ConnectionState connectionState, string streamId, Func<Task> createAndConsumeStream, CancellationTokenSource cts)
    {
        ...
        try
        {
            ...
            if (_state.IsConnectionActive())
            {
                ...
                await SendRpcMessage(connectionState, CompletionMessage.WithError(streamId, responseError), cancellationToken: default).ConfigureAwait(false);
            }
            else
            {
    ...
    private async Task<object?> InvokeCoreAsyncCore(string methodName, Type returnType, object?[] args, IDictionary<string, string>? headers, CancellationToken cancellationToken)
    {
        var readers = default(Dictionary<string, object>);
        ...
        try
        {
            ...
            await InvokeCore(connectionState, methodName, irq, args, streamIds?.ToArray(), headers, cancellationToken).ConfigureAwait(false);
    ...
    private async Task InvokeCore(ConnectionState connectionState, string methodName, InvocationRequest irq, object?[] args, string[]? streams, IDictionary<string, string>? headers, CancellationToken cancellationToken)
    {
        ...
        var invocationMessage = new InvocationMessage(irq.InvocationId, methodName, args, streams) { Headers = headers };
        ...
        try
        {
            await SendRpcMessage(connectionState, invocationMessage, cancellationToken).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    private async Task InvokeStreamCore(ConnectionState connectionState, string methodName, InvocationRequest irq, object?[] args, string[]? streams, IDictionary<string, string>? headers, CancellationToken cancellationToken)
    {
        ...
        var invocationMessage = new StreamInvocationMessage(irq.InvocationId, methodName, args, streams) {  Headers = headers };
    ...
    private async Task SendRpcMessage(ConnectionState connectionState, RpcMessage hubMessage,  CancellationToken cancellationToken = default)
    {
        _state.AssertConnectionValid();
    ...
    private async Task SendCoreAsyncCore(string methodName, object?[] args, IDictionary<string, string>? headers, CancellationToken cancellationToken)
    {
        var readers = default(Dictionary<string, object>);
        ...
        try
        {
            ...
            var invocationMessage = new InvocationMessage(null, methodName, args, streamIds?.ToArray()) { Headers = headers };
            await SendRpcMessage(connectionState, invocationMessage, cancellationToken).ConfigureAwait(false);
    ...
    private async Task SendWithLock(ConnectionState expectedConnectionState, RpcMessage message, CancellationToken cancellationToken, [CallerMemberName] string callerName = "")
    {
        CheckDisposed();
        ...
        try
        {
            ...
            await SendRpcMessage(connectionState, message, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            _state.ReleaseConnectionLock();
    ...
    private async Task<CloseMessage?> ProcessMessagesAsync(RpcMessage message, ConnectionState connectionState, ChannelWriter<InvocationMessage> invocationMessageWriter)
    {
...
#pragma warning restore CA2252 // This API requires opting into preview features
    ...
    private async Task HandshakeAsync(ConnectionState startingConnectionState, int protocolVersion, CancellationToken cancellationToken)
    {
        ...
        Log.SendingRpcHandshake(_logger);
                    ...
                    else if (!buffer.IsEmpty)
                    {
                        ...
                        while (_protocol.TryParseMessage(ref buffer, connectionState, out var message))
                        {
                            ...
                            if (closeMessage != null)
                            {
                                ...
                                {
                                    connectionState.CloseException = new RpcException($"The server closed the connection with the following error: {closeMessage.Error}");
                                }
    ...
    private async Task HandleConnectionClose(ConnectionState connectionState)
    {
        ...
        try
        {
            ...
            if (connectionState.Stopping || _reconnectPolicy == null)
            {
                ...
                _state.ChangeState(RpcConnectionState.Connected, RpcConnectionState.Disconnected);
    ...
    private async Task ReconnectAsync(Exception? closeException)
    {
        ...
        _state.ChangeState(RpcConnectionState.Connected, RpcConnectionState.Reconnecting);
        ...
        while (nextRetryDelay != null)
        {
            ...
            catch (OperationCanceledException ex)
            {
                ...
                try
                {
                    _state.ChangeState(RpcConnectionState.Reconnecting, RpcConnectionState.Disconnected);
            ...
            try
            {
                ...
                _state.ChangeState(RpcConnectionState.Reconnecting, RpcConnectionState.Connected);
...
#pragma warning restore CA1822
    ...
    private sealed class ConnectionState : IInvocationBinder
    {
        ...
        // This means either RpcConnection.DisposeAsync/StopAsync was called OR a CloseMessage with AllowReconnects set to false was received.
        ...
        public ConnectionState(ConnectionContext connection, RpcConnection hubConnection)
        {
            Connection = connection;

            _rpcConnection = hubConnection;
            _rpcConnection._logScope.ConnectionId = connection.ConnectionId;

            _logger = _rpcConnection._logger;
...
#pragma warning disable CA2252 // This API requires opting into preview features
            if (Connection.Features.Get<IStatefulReconnectFeature>() is IStatefulReconnectFeature feature)
            {
                _messageBuffer = new MessageBuffer(connection, hubConnection._protocol,
                    _rpcConnection._serviceProvider.GetService<IOptions<RpcConnectionOptions>>()?.Value.StatefulReconnectBufferSize
...
#pragma warning restore CA2252 // This API requires opting into preview features
        ...
        {
            _rpcConnection._state.AssertInConnectionLock();
            ...
            _rpcConnection._logScope.ConnectionId = null;
        ...
        public ValueTask<FlushResult> WriteAsync(RpcMessage message, CancellationToken cancellationToken)
        {
            Debug.Assert(_messageBuffer is not null);
            return _messageBuffer.WriteAsync(message, cancellationToken);
        }

        public bool ShouldProcessMessage(RpcMessage message)
        {
            if (UsingAcks())
            {
                if (!_messageBuffer.ShouldProcessMessage(message))
                {
                    Log.DroppingMessage(_logger, ((RpcInvocationMessage)message).GetType().Name, ((RpcInvocationMessage)message).InvocationId);
                    return false;
                }
            }
            return true;
        ...
        {
            Volatile.Write(ref _nextActivationSendPing, (DateTime.UtcNow + _rpcConnection.KeepAliveInterval).Ticks);
        }

        public void ResetTimeout()
        {
            Volatile.Write(ref _nextActivationServerTimeout, (DateTime.UtcNow + _rpcConnection.ServerTimeout).Ticks);
        }
        ...
        internal async Task RunTimerActions()
        {
            ...
            {
                if (!_rpcConnection._state.TryAcquireConnectionLock())
                {
                ...
                try
                {
                    if (_rpcConnection._state.CurrentConnectionStateUnsynchronized != null)
                    {
                        SafeAssert(ReferenceEquals(_rpcConnection._state.CurrentConnectionStateUnsynchronized, this),
                            "Something reset the connection state before the timer loop completed!");

                        await _rpcConnection.SendRpcMessage(this, PingMessage.Instance).ConfigureAwait(false);
                    }
                }
                catch
                {
                ...
                {
                    _rpcConnection._state.ReleaseConnectionLock();
                }
            }
        }
        ...
        internal void OnServerTimeout()
        {
            CloseException = new TimeoutException(
                $"Server timeout ({_rpcConnection.ServerTimeout.TotalMilliseconds:0.00}ms) elapsed without receiving a message from the server.");
            Connection.Transport.Input.CancelPendingRead();
        }
        ...
        {
            if (!_rpcConnection._handlers.TryGetValue(methodName, out var invocationHandlerList))
            {
    ...
    private sealed class ReconnectingConnectionState
    {
        ...
        public RpcConnectionState OverallState { get; private set; }

        public CancellationTokenSource StopCts { get; set; } = new ();

        public Task ReconnectTask { get; set; } = Task.CompletedTask;

        public void ChangeState(RpcConnectionState expectedState, RpcConnectionState newState)
        {
            if (!TryChangeState(expectedState, newState))
            {
                Log.StateTransitionFailed(_logger, expectedState, newState, OverallState);
                throw new InvalidOperationException($"The RpcConnection failed to transition from the '{expectedState}' state to the '{newState}' state because it was actually in the '{OverallState}' state.");
            }
        }

        public bool TryChangeState(RpcConnectionState expectedState, RpcConnectionState newState)
        {
