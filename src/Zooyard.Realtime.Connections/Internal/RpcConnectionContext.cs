...
using Zooyard.Realtime.Connections.Internal.Transports;
using Zooyard.Realtime.Features;
using Zooyard.Realtime.Internal;

namespace Zooyard.Realtime.Connections.Internal;

internal sealed partial class RpcConnectionContext : ConnectionContext,
...
#pragma warning restore CA2252 // This API requires opting into preview features
{
    ...
    /// </summary>
    public RpcConnectionContext(string connectionId, string connectionToken, ILogger logger, MetricsContext metricsContext,
...
#pragma warning restore CA2252 // This API requires opting into preview features
    ...
    public long? LastSeenTicksIfInactive
    {
        get
        {
            lock (_stateLock)
            {
                return Status == RpcConnectionStatus.Inactive ? LastSeenTicks : null;
            }
        }
    }

    public RpcConnectionStatus Status { get; set; } = RpcConnectionStatus.Inactive;
    ...
    public async Task DisposeAsync(bool closeGracefully = false)
    {
        ...
        try
        {
            lock (_stateLock)
            {
                if (Status == RpcConnectionStatus.Disposed)
                {
                    disposeTask = _disposeTcs.Task;
                }
                else
                {
                    Status = RpcConnectionStatus.Disposed;
    ...
    private async Task WaitOnTasks(Task applicationTask, Task transportTask, bool closeGracefully)
    {
        try
        {
            ...
            if (!closeGracefully)
            {
                ...
                if (TransportType == Microsoft.AspNetCore.Http.Connections.HttpTransportType.WebSockets)
                {
            ...
            if (result == applicationTask)
            {
                ...
                try
                {
                    Log.WaitingForTransport(_logger, TransportType.ToString());
                ...
                {
                    Log.TransportComplete(_logger, TransportType.ToString());
        ...
        ILogger dispatcherLogger)
    {
        lock (_stateLock)
        {
            if (Status == RpcConnectionStatus.Inactive)
            {
                Status = RpcConnectionStatus.Active;
    ...
    {
        if (Status == RpcConnectionStatus.Active)
        {
            RpcConnectionDispatcher.Log.ConnectionAlreadyActive(dispatcherLogger, ConnectionId, HttpContext!.TraceIdentifier);

            // Reject the request with a 409 conflict
            nonClonedContext.Response.StatusCode = Microsoft.AspNetCore.Http.StatusCodes.Status409Conflict;
            //nonClonedContext.Response.ContentType = "text/plain";
        }
        else
        {
            Debug.Assert(Status == RpcConnectionStatus.Disposed);

            RpcConnectionDispatcher.Log.ConnectionDisposed(dispatcherLogger, ConnectionId);

            // Connection was disposed
            nonClonedContext.Response.StatusCode = Microsoft.AspNetCore.Http.StatusCodes.Status404NotFound;
            //nonClonedContext.Response.ContentType = "text/plain";
        }
    }
    ...
    internal async Task<bool> CancelPreviousPoll(HttpContext context)
    {
        ...
        using (cts)
        {
            ...
            if (UseStatefulReconnect && TransportType == Microsoft.AspNetCore.Http.Connections.HttpTransportType.WebSockets)
            {
    ...
    internal SetTransportState TrySetTransport(Microsoft.AspNetCore.Http.Connections.HttpTransportType transportType, RpcConnectionsMetrics metrics)
    {
        lock (_stateLock)
        {
            if (TransportType == Microsoft.AspNetCore.Http.Connections.HttpTransportType.None)
            {
                ...
                if (RpcConnectionsEventSource.Log.IsEnabled() || MetricsContext.ConnectionDurationEnabled)
                {
                    StartTimestamp = Stopwatch.GetTimestamp();
                }

                RpcConnectionsEventSource.Log.ConnectionStart(ConnectionId);
    ...
    public void MarkInactive()
    {
        lock (_stateLock)
        {
            if (Status == RpcConnectionStatus.Active)
            {
                Status = RpcConnectionStatus.Inactive;
                LastSeenTicks = Environment.TickCount64;
            }
        }
    }
    ...
    private async Task ExecuteApplication(ConnectionDelegate connectionDelegate)
    {
        ...
        Debug.Assert(TransportType != Microsoft.AspNetCore.Http.Connections.HttpTransportType.None, "Transport has not been initialized yet");
...
#pragma warning restore CA2252 // This API requires opting into preview features
    ...
    private static partial class Log
    {
        ...
        public static partial void WaitingForTransport(ILogger logger, string transportType);

        [LoggerMessage(5, LogLevel.Trace, "{TransportType} transport complete.", EventName = "TransportComplete")]
        public static partial void TransportComplete(ILogger logger, string transportType);
        ...
        public static partial void ShuttingDownTransportAndApplication(ILogger logger, string transportType);
        ...
        public static partial void WaitingForTransportAndApplication(ILogger logger, string transportType);
        ...
        public static partial void TransportAndApplicationComplete(ILogger logger, string transportType);
