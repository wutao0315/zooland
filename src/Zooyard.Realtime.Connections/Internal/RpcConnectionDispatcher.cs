...
internal sealed partial class RpcConnectionDispatcher
{
    ...
    private readonly RpcConnectionManager _manager;
    ...
    public RpcConnectionDispatcher(RpcConnectionManager manager, ILoggerFactory loggerFactory, RpcConnectionsMetrics metrics)
    {
        _manager = manager;
        _loggerFactory = loggerFactory;
        _metrics = metrics;
        _logger = _loggerFactory.CreateLogger<RpcConnectionDispatcher>();
    }
    ...
    public async Task ExecuteAsync(HttpContext context, HttpConnectionDispatcherOptions options, ConnectionDelegate connectionDelegate)
    {
        ...
        RpcConnectionContext? connectionContext = null;
        ...
        {
            //if (Microsoft.AspNetCore.Http.HttpMethods.IsPost(context.Request.Method))
            //{
            //    // POST /{path}
            //    await ProcessSend(context);
            //}
            //else if (Microsoft.AspNetCore.Http.HttpMethods.IsGet(context.Request.Method) || Microsoft.AspNetCore.Http.HttpMethods.IsConnect(context.Request.Method))
            //{
            //    // GET /{path}
            //    await ExecuteAsync(context, connectionDelegate, options, logScope);
            //}
            //else if (Microsoft.AspNetCore.Http.HttpMethods.IsDelete(context.Request.Method))
            //{
            //    // DELETE /{path}
    ...
    //public async Task ExecuteNegotiateAsync(RpcContext context, RpcConnectionDispatcherOptions options)
    //{
    ...
    private async Task ExecuteAsync(HttpContext context, ConnectionDelegate connectionDelegate, HttpConnectionDispatcherOptions options, ConnectionLogScope logScope)
    {
        ...
        //if (headers.Accept?.Contains(new MediaTypeHeaderValue("text/event-stream")) == true)
        //{
        ...
        //    if (!await EnsureConnectionStateAsync(connection, context, Microsoft.AspNetCore.Http.Connections.HttpTransportType.ServerSentEvents, supportedTransports, logScope))
        //    {
        ...
        //    var transport = Microsoft.AspNetCore.Http.Connections.HttpTransportType.LongPolling;
        //    if (context.WebSockets.IsWebSocketRequest)
        //    {
        //        transport = Microsoft.AspNetCore.Http.Connections.HttpTransportType.WebSockets;
        ...
        //    if (connection.TransportType != Microsoft.AspNetCore.Http.Connections.HttpTransportType.WebSockets || connection.UseStatefulReconnect)
        //    {
        ...
        //    switch (transport)
        //    {
        //        case Microsoft.AspNetCore.Http.Connections.HttpTransportType.None:
        //            break;
        //        case Microsoft.AspNetCore.Http.Connections.HttpTransportType.WebSockets:
        ...
        //            }
        //            break;
        //        case Microsoft.AspNetCore.Http.Connections.HttpTransportType.LongPolling:
        ...
        //    context.Features.Get<IRpcRequestTimeoutFeature>()?.DisableTimeout();
        ...
        //            if (context.Response.StatusCode == Microsoft.AspNetCore.Http.StatusCodes.Status204NoContent)
        //            {
        ...
        //                await _manager.DisposeAndRemoveAsync(connection, closeGracefully: false, RpcConnectionStopStatus.NormalClosure);
        //            }
        //            else
        //            {
        //                if (transport != Microsoft.AspNetCore.Http.Connections.HttpTransportType.LongPolling)
        //                {
        //                    await _manager.DisposeAndRemoveAsync(connection, closeGracefully: false, RpcConnectionStopStatus.NormalClosure);
        //                }
        //                else
        ...
        //            {
        //                await _manager.DisposeAndRemoveAsync(connection, closeGracefully: true, RpcConnectionStopStatus.NormalClosure);
        //            }
        //            else
        //            {
        //                // Only allow repoll if we aren't removing the connection.
    ...
    private async Task DoPersistentConnection(RpcConnectionContext connection)
    {
    ...
    //private async Task ProcessNegotiate(RpcContext context, RpcConnectionDispatcherOptions options, ConnectionLogScope logScope)
    //{
    ...
    //    RpcConnectionContext? connection = null;
    //    if (error == null)
    //    {
    ...
    //private static void WriteNegotiatePayload(IBufferWriter<byte> writer, string? connectionId, string? connectionToken, RpcContext context, RpcConnectionDispatcherOptions options,
    ...
    //    if ((options.Transports & Microsoft.AspNetCore.Http.Connections.HttpTransportType.WebSockets) != 0 && ServerHasWebSockets(context.Features))
    //    {
    //        response.AvailableTransports.Add(_webSocketAvailableTransport);
    //    }

    //    if ((options.Transports & Microsoft.AspNetCore.Http.Connections.HttpTransportType.ServerSentEvents) != 0)
    //    {
    //        response.AvailableTransports.Add(_serverSentEventsAvailableTransport);
    //    }

    //    if ((options.Transports & Microsoft.AspNetCore.Http.Connections.HttpTransportType.LongPolling) != 0)
    //    {
    ...
    private async Task ProcessSend(HttpContext context)
    {
        ...
        if (connection.TransportType == Microsoft.AspNetCore.Http.Connections.HttpTransportType.WebSockets)
        {
        ...
        try
        {
            if (connection.Status == RpcConnectionStatus.Disposed)
            {
    ...
    private async Task ProcessDeleteAsync(HttpContext context)
    {
        ...
        if (connection.TransportType != Microsoft.AspNetCore.Http.Connections.HttpTransportType.LongPolling)
        {
        ...
        connection.DisposeAndRemoveTask = _manager.DisposeAndRemoveAsync(connection, closeGracefully: false, RpcConnectionStopStatus.NormalClosure);

        context.Response.StatusCode = Microsoft.AspNetCore.Http.StatusCodes.Status202Accepted;
        //context.Response.ContentType = "text/plain";
    }

    private async Task<bool> EnsureConnectionStateAsync(RpcConnectionContext connection, HttpContext context, Microsoft.AspNetCore.Http.Connections.HttpTransportType transportType, Microsoft.AspNetCore.Http.Connections.HttpTransportType supportedTransports, ConnectionLogScope logScope)
    {
        ...
        {
            case RpcConnectionContext.SetTransportState.Success:
                break;

            case RpcConnectionContext.SetTransportState.AlreadyActive:
            ...
            case RpcConnectionContext.SetTransportState.CannotChange:
                //context.Response.ContentType = "text/plain";
                context.Response.StatusCode = Microsoft.AspNetCore.Http.StatusCodes.Status400BadRequest;
        ...
        if (transportType == Microsoft.AspNetCore.Http.Connections.HttpTransportType.LongPolling)
        {
            ...
            else
            {
                ...
                if (context.User.Identity is not WindowsIdentity)
                {
    ...
    private static void UpdateExpiration(RpcConnectionContext connection, HttpContext context)
    {
    ...
    private static void CloneHttpContext(HttpContext context, RpcConnectionContext connection)
    {
    ...
    private async Task<RpcConnectionContext?> GetConnectionAsync(HttpContext context)
    {
        var connectionToken = GetConnectionToken(context);
    ...
    private async Task<RpcConnectionContext?> GetOrCreateConnectionAsync(HttpContext context, HttpConnectionDispatcherOptions options)
    {
        var connectionToken = GetConnectionToken(context);
        RpcConnectionContext? connection;
    ...
    private RpcConnectionContext CreateConnection(HttpConnectionDispatcherOptions options, int clientProtocolVersion = 0, bool useStatefulReconnect = false)
    {
