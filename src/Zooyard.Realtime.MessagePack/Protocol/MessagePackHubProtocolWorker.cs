...
internal abstract class MessagePackHubProtocolWorker
{
    ...
    private RpcMessage? ParseMessage(ref MessagePackReader reader, IInvocationBinder binder)
    {
    ...
    private RpcMessage CreateInvocationMessage(ref MessagePackReader reader, IInvocationBinder binder, int itemCount)
    {
    ...
    private RpcMessage CreateStreamInvocationMessage(ref MessagePackReader reader, IInvocationBinder binder, int itemCount)
    {
        var headers = ReadHeaders(ref reader);
        var invocationId = ReadInvocationId(ref reader);
    ...
    private RpcMessage CreateStreamItemMessage(ref MessagePackReader reader, IInvocationBinder binder)
    {
        var headers = ReadHeaders(ref reader);
        var invocationId = ReadInvocationId(ref reader);
    ...
    private static T ApplyHeaders<T>(IDictionary<string, string>? source, T destination) where T : RpcInvocationMessage
    {
    ...
    private void WriteMessageCore(RpcMessage message, ref MessagePackWriter writer)
    {
        switch (message)
        {
    ...
    private void WriteInvocationMessage(InvocationMessage message, ref MessagePackWriter writer)
    {
        ...
        writer.Write(RpcProtocolConstants.InvocationMessageType);
    ...
    private void WriteStreamInvocationMessage(StreamInvocationMessage message, ref MessagePackWriter writer)
    {
        ...
        writer.Write(RpcProtocolConstants.StreamInvocationMessageType);
        PackHeaders(message.Headers, ref writer);
        writer.Write(message.InvocationId);
        writer.Write(message.Target);
    ...
    private void WriteStreamingItemMessage(StreamItemMessage message, ref MessagePackWriter writer)
    {
        ...
        writer.Write(RpcProtocolConstants.StreamItemMessageType);
        PackHeaders(message.Headers, ref writer);
        writer.Write(message.InvocationId);
    ...
    private void WriteCompletionMessage(CompletionMessage message, ref MessagePackWriter writer)
    {
        ...
        writer.Write(RpcProtocolConstants.CompletionMessageType);
        PackHeaders(message.Headers, ref writer);
        writer.Write(message.InvocationId);
    ...
    private static void WriteCancelInvocationMessage(CancelInvocationMessage message, ref MessagePackWriter writer)
    {
        ...
        writer.Write(RpcProtocolConstants.CancelInvocationMessageType);
        PackHeaders(message.Headers, ref writer);
        writer.Write(message.InvocationId);
    }
    ...
    {
        writer.WriteArrayHeader(3);
        writer.Write(RpcProtocolConstants.CloseMessageType);
    ...
    private static void WritePingMessage(ref MessagePackWriter writer)
    {
        ...
        writer.Write(RpcProtocolConstants.PingMessageType);
    }

    private static void WriteAckMessage(AckMessage message, ref MessagePackWriter writer)
    {
        writer.WriteArrayHeader(2);
        writer.Write(RpcProtocolConstants.AckMessageType);
    ...
    {
        writer.WriteArrayHeader(2);
        writer.Write(RpcProtocolConstants.SequenceMessageType);
        writer.Write(message.SequenceId);
    }
