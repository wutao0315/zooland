...
public class THeaderProcessor : ITAsyncProcessor
{
    ...
    public async Task<bool> ProcessAsync(TProtocol iprot, TProtocol oprot, CancellationToken cancellationToken = default)
    {
        ...
        var activitySource = new ActivitySource(ActivitySourceName, version);
        //bool hasDiagnosticListener = false;
        using var activity = StartActivity(headInfo, activitySource);
    ...
    private static Activity? StartActivity(IDictionary<string, string>? headers, ActivitySource source)
    {
        ...
        headers.TryGetValue(HeaderNames.TraceState, out traceStateString);

        if (headers.TryGetValue(HeaderNames.TraceParent, out var traceParentHeaderName)
        ...
        if (headers.TryGetValue(HeaderNames.RequestId, out var requestId))
        {
            parentId = requestId!;
        }

        var activity = activityContext == null ? source.StartActivity(ActivityName, ActivityKind.Server, parentId) : source.StartActivity(ActivityName, ActivityKind.Server, activityContext.Value);
        return activity;
    }
