...
namespace Zooyard.WebSocketsImpl.Internal;
...
#pragma warning restore CA2252 // This API requires opting into preview features
    ...
    public WebSocketsTransport(WebSocketConnectionOptions httpConnectionOptions, ILoggerFactory loggerFactory, Func<Task<string?>> accessTokenProvider, HttpClient? httpClient,
        bool useStatefulReconnect = false)
    {
        _useStatefulReconnect = useStatefulReconnect;
        _logger = (loggerFactory ?? NullLoggerFactory.Instance).CreateLogger<WebSocketsTransport>();
        _httpConnectionOptions = httpConnectionOptions ?? new WebSocketConnectionOptions();
            ...
            )
        {
            ...
            if (!string.IsNullOrWhiteSpace(accessToken))
            {
                ...
                if (OperatingSystem.IsBrowser())
                {
                    ...
                    url = Zooyard.WebSocketsImpl.Connections.Internal.Utils.AppendQueryString(url, accessTokenEncoded);
                }
                else
                {
