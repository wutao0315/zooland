...
/// <summary>
/// Options used to configure a <see cref="WebSocketConnection"/> instance.
/// </summary>
public class WebSocketConnectionOptions
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="WebSocketConnectionOptions"/> class.
    /// </summary>
    ...
    public long TransportMaxBufferSize
    {
        ...
        set
        {
            if (value < 0)
            {
                throw new ArgumentOutOfRangeException(null, value, $"'null' must be a non-negative value.");
            }
    ...
    /// <summary>
    /// Gets or sets the default <see cref="TransferFormat" /> to use if <see cref="WebSocketConnection.StartAsync(CancellationToken)"/>
    /// is called instead of <see cref="WebSocketConnection.StartAsync(TransferFormat, CancellationToken)"/>.
    /// </summary>
    ...
    /// has been applied.
    /// <para />
    /// If <c>ClientWebSocketOptions.HttpVersion</c> is set to <c>2.0</c> or higher, some options like <see cref="ClientWebSocketOptions.Cookies"/> will not be applied. Instead use <see cref="Cookies"/> or the corresponding option on <see cref="WebSocketConnectionOptions"/>.
    /// </remarks>
    [UnsupportedOSPlatform("browser")]
    ...
    /// Also preserves the <see cref="WebSocketConnection.ConnectionId"/> when the reconnect is successful.
    /// </summary>
    /// <remarks>
