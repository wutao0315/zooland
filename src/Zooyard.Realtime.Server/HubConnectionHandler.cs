...
using Log = Zooyard.Realtime.Server.HubConnectionHandlerLog;
...
public class HubConnectionHandler<THub> : ConnectionHandler where THub : Hub
{
    ...
    private readonly IRpcProtocolResolver _protocolResolver;
    ...
    public HubConnectionHandler(HubLifetimeManager<THub> lifetimeManager,
                                IRpcProtocolResolver protocolResolver,
    ...
    private async Task HubOnDisconnectedAsync(HubConnectionContext connection, Exception? exception)
    {
        ...
        if (connection.CloseMessage is not null)
        {
            ...
            if (connection.CloseMessage.Error is not null)
            {
                ...
                disconnectException = new RpcException(connection.CloseMessage.Error);
            }
        }
