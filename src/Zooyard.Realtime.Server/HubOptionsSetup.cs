...
public class HubOptionsSetup : IConfigureOptions<HubOptions>
{
    ...
    /// </summary>
    /// <param name="protocols">The list of <see cref="IRpcProtocol"/>s that are from Dependency Injection.</param>
    public HubOptionsSetup(IEnumerable<IRpcProtocol> protocols)
    {
        foreach (var hubProtocol in protocols)
        {
            if (hubProtocol.GetType().CustomAttributes.Where(a => a.AttributeType.FullName == "Zooyard.Realtime.Server.Internal.NonDefaultHubProtocolAttribute").Any())
            {
                continue;
            }
