...
using Log = Zooyard.Realtime.Server.Internal.DefaultHubDispatcherLog;
...
internal sealed partial class DefaultHubDispatcher<THub> : HubDispatcher<THub> where THub : Hub
{
    ...
    public override Task DispatchMessageAsync(HubConnectionContext connection, RpcMessage hubMessage)
    {
        ...
        {
            Log.DroppingMessage(_logger, hubMessage.GetType().Name, (hubMessage as RpcInvocationMessage)?.InvocationId ?? "(null)");
    ...
    private Task ProcessInvocation(HubConnectionContext connection,
        RpcMethodInvocationMessage hubMethodInvocationMessage, bool isStreamResponse)
    {
    ...
    private async Task<bool> Invoke(HubMethodDescriptor descriptor, HubConnectionContext connection,
        RpcMethodInvocationMessage hubMethodInvocationMessage, bool isStreamResponse, bool isStreamCall)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    var ex = new RpcException($"Client sent {clientStreamLength} stream(s), Hub method expects {serverStreamLength}.");
                ...
                else
                {
                    ...
                    static async Task ExecuteInvocation(DefaultHubDispatcher<THub> dispatcher,
                                                        ...
                                                        RpcMethodInvocationMessage hubMethodInvocationMessage,
                                                        bool isStreamCall)
                    {
    ...
    private static ValueTask CleanupInvocation(HubConnectionContext connection, RpcMethodInvocationMessage hubMessage, IHubActivator<THub>? hubActivator,
    ...
    private async Task StreamAsync(string invocationId, HubConnectionContext connection, object?[] arguments, AsyncServiceScope scope,
        IHubActivator<THub> hubActivator, THub hub, CancellationTokenSource? streamCts, RpcMethodInvocationMessage hubMethodInvocationMessage, HubMethodDescriptor descriptor)
    {
        string? error = null;
        ...
        {
            // If the streaming method was canceled we don't want to send a RpcException message - this is not an error case
    ...
    private ValueTask<object?> ExecuteMethod(string hubMethodName, Hub hub, object?[] arguments)
    {
        ...
        {
            throw new RpcException($"Unknown hub method '{hubMethodName}'");
        }
    ...
    private async Task<bool> ValidateInvocationMode(HubMethodDescriptor hubMethodDescriptor, bool isStreamResponse,
        RpcMethodInvocationMessage hubMethodInvocationMessage, HubConnectionContext connection)
    {
    ...
    private void ReplaceArguments(HubMethodDescriptor descriptor, RpcMethodInvocationMessage hubMethodInvocationMessage, bool isStreamCall,
    ...
    public override IReadOnlyList<Type> GetParameterTypes(string methodName)
    {
        ...
        {
            throw new RpcException("Method does not exist.");
        }
